...
with gr.Blocks() as app:
    gr.Markdown(
        """
        # Welcome!
        Select a Hugging Face model or tag and deploy it on different ports
        
        **Note**: _[vLLM supported models list](https://docs.vllm.ai/en/latest/models/supported_models.html)_        
        """)


    demotest = gr.Interface.from_pipeline(pipeline("image-classification", model="julien-c/hotdog-not-hotdog"))
    ...
    timer_dl_box = gr.Textbox(label="Dowmload progress:",visible=True)
